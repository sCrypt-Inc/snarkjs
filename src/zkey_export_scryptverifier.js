...
export async function exportScryptVerifier(zKeyName, templates, logger) {
...
export async function exportScryptTSVerifier(zKeyName, templates, logger) {
    ...
    if ((vKey.curve == "bn128")  && vKey.protocol == "groth16") {
        ...
        let resStr = zokratesProvider.computeMillerBetaAlpha(zokrates_vk);
        let vk_miller_alphabeta_12 = resStr.replaceAll('{', '').replaceAll('}', '').replaceAll(',', '').split(/[\s\n]+/).filter(e => e);
        vk_miller_alphabeta_12 = vk_miller_alphabeta_12.map((x) => {
          return BigInt(x).toString(10)
        })
        
        vKey.vk_miller_alphabeta_12 = vk_miller_alphabeta_12;
    }
